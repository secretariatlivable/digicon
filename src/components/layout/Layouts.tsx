...
// Supabase Auth is the single browser authentication source of truth.
          ...
          <nav className="hidden items-center gap-1 md:flex" aria-label="Site navigation">
            {PUBLIC_LINKS.map((link) => (
              <Link
                ...
                data-testid={`public-nav-${link.label.toLowerCase().replace(/\s+/g, "-")}`}
              >
                {link.label}
              </Link>
            ))}
            ...
            <nav className="flex flex-wrap gap-x-4 gap-y-2 sm:hidden" aria-label="Legal and support">
              {FOOTER_LINKS.map((link) => (
                <Link
                  ...
                  data-testid={`footer-mobile-${link.label.toLowerCase().replace(/\s+/g, "-")}`}
                >
                  {link.label}
                </Link>
              ))}
...
export function Protected({ children, adminOnly = false, bare = false }: { children: ReactNode; adminOnly?: boolean; bare?: boolean }) {
