      ...
      <div className="mx-auto max-w-5xl px-4 py-12">
        <div className="text-center">
          ...
          <p className="dense mx-auto mt-3 max-w-xl text-muted-foreground">
            Use the core relationship workspace for free. Upgrade when DigiCon becomes the system of record for your professional network.
          </p>
        </div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          <div className="glass rounded-2xl p-6" data-testid="plan-free">
            ...
            <p className="metric mt-1 text-3xl">$0</p>
        ...
        <section className="glass mt-10 rounded-2xl p-5" aria-labelledby="paypal-plans-title">
          ...
          />
          <p id="paypal-plans-title" className="dense mt-3 text-sm text-muted-foreground">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {PAYPAL_PLANS.map((plan) => (
              <div key={plan.id} className="metal-edge rounded-2xl p-5" data-testid={`paypal-plan-${plan.id}`}>
                <p className="label-caps">{plan.name}</p>
                <p className="metric mt-1 text-2xl gold-text">{plan.price}</p>
                <p className="dense mb-4 mt-1 text-sm text-muted-foreground">{plan.description}</p>
                <PayPalSubscriptionButton planId={plan.id} />
              </div>
            ))}
          </div>
        </section>
