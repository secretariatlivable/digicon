...
import { User, Globe, Bell, Palette, Users, CreditCard, Check, Save } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const [profileData, setProfileData] = useState({
    full_name: '', company_name: '', email: '', region: 'metro-manila',
  });
  const [prefs, setPrefs] = useState({
    language: 'en' as 'en' | 'fil', notifications: { email: true, push: true, sms: false }, theme: 'dark' as 'light' | 'dark',
  });
        ...
        <GlassCard variant="regular" className="p-6 max-w-2xl animate-fade-in">
          ...
          <div className="space-y-4">
            <div>
              ...
              <GlassInput value={profileData.full_name} onChange={(e) => setProfileData({ ...profileData, full_name: e.target.value })} />
            </div>
            <div>
              <GlassLabel>{t('settings.company')}</GlassLabel>
              <GlassInput value={profileData.company_name} onChange={(e) => setProfileData({ ...profileData, company_name: e.target.value })} />
            </div>
            <div>
            ...
            <div>
              ...
              <GlassSelect value={profileData.region} onChange={(e) => setProfileData({ ...profileData, region: e.target.value })}>
          ...
          <div className="flex items-center gap-3 mt-6">
            <GlassButton onClick={saveProfile} disabled={saving}>
              {saving ? <Spinner /> : saved ? <><Check className="inline mr-2 w-4 h-4" /> {t('settings.saved')}</> : <><Save className="inline mr-2 w-4 h-4" /> {t('settings.save')}</>}
            </GlassButton>
          </div>
        </GlassCard>
      )}
            ...
            <div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 p-4 rounded-glass-md border transition-all ${prefs.language === 'en' ? 'glass-regular border-digicon-primary/50' : 'glass-thin border-white/10'}`}
                >
                  <span className="text-white font-medium">English</span>
                </button>
                <button
                  onClick={() => setPrefs({ ...prefs, language: 'fil' })}
                  className={`flex-1 p-4 rounded-glass-md border transition-all ${prefs.language === 'fil' ? 'glass-regular border-digicon-primary/50' : 'glass-thin border-white/10'}`}
                >
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <label key={item.key} className="flex items-center gap-3 cursor-pointer p-3 rounded-glass-sm glass-thin hover:bg-white/5 transition-all">
                    <input
                      type="checkbox"
                      checked={prefs.notifications[item.key]}
                      onChange={(e) => setPrefs({ ...prefs, notifications: { ...prefs.notifications, [item.key]: e.target.checked } })}
            ...
            <div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 p-4 rounded-glass-md border transition-all ${prefs.theme === 'light' ? 'glass-regular border-digicon-primary/50' : 'glass-thin border-white/10'}`}
                >
                  <span className="text-white font-medium">{t('settings.light')}</span>
                </button>
                <button
                  onClick={() => setPrefs({ ...prefs, theme: 'dark' })}
                  className={`flex-1 p-4 rounded-glass-md border transition-all ${prefs.theme === 'dark' ? 'glass-regular border-digicon-primary/50' : 'glass-thin border-white/10'}`}
                >
                  <span className="text-white font-medium">{t('settings.dark')}</span>
                </button>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'team' && (
        <GlassCard variant="regular" className="p-6 max-w-2xl animate-fade-in">
          <div className="flex items-center justify-between mb-6">
            ...
            <GlassButton size="sm" variant="secondary">{t('settings.invite')}</GlassButton>
          </div>
          <div className="glass-thin rounded-glass-md p-6 text-center">
            <Users className="w-10 h-10 text-white/20 mx-auto mb-3" />
            <p className="text-sm text-white/50">{t('settings.noTeam')}</p>
            <p className="text-xs text-white/30 mt-2">Team features available on Growth and Enterprise plans.</p>
          </div>
        </GlassCard>
      )}
        ...
        <GlassCard variant="regular" className="p-6 max-w-2xl animate-fade-in">
          ...
          <div className="glass-thin rounded-glass-md p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="px-3 py-1 rounded-full bg-digicon-eco/20 text-digicon-eco text-xs font-medium">Active</span>
            </div>
            <p className="text-sm text-white/50 mb-4">₱199/month - 3 cards, 100 contacts, basic analytics</p>
            <GlassButton variant="secondary" size="sm">{t('settings.upgrade')}</GlassButton>
          </div>
        </GlassCard>
      )}
    </AppLayout>
