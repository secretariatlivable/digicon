-- DigiCon production security + subscriptions foundation.
-- Apply this migration after the existing core schema.

create extension if not exists pgcrypto;

-- Subscription state used by the application as the authoritative billing state.
create table if not exists public.subscriptions (
  ...
  status text not null check (status in ('APPROVAL_PENDING', 'ACTIVE', 'SUSPENDED', 'CANCELLED', 'EXPIRED')),
...
-- Keep all public card reads intentionally narrow: only active cards.
...
-- Owner policies still apply for writes.
drop policy if exists "insert_own_cards" on public.business_cards;
...
drop policy if exists "update_own_cards" on public.business_cards;
create policy "update_own_cards"
on public.business_cards
for update
to authenticated
using (auth.uid() = user_id)
...
drop policy if exists "delete_own_cards" on public.business_cards;
...
-- Anonymous lead capture through RPC.
...
revoke all on function public.capture_public_contact(uuid, text, text, text, boolean)
from public;

grant execute on function public.capture_public_contact(uuid, text, text, text, boolean)
to anon, authenticated;

-- Make the RPC atomic for eco contact counts.
create or replace function public.increment_eco_contacts_saved(p_user_id uuid)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  insert into public.eco_stats (user_id, contacts_saved)
  values (p_user_id, 1)
  on conflict (user_id)
  do update set
...
-- New-user profile provisioning. Avoids a client-only race around email confirmation.
